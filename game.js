...
const state={
  ...
  speed:4.6,baseSpeed:4.6,gravity:1900,jump:-720,dino:{x:70,y:0,w:44,h:48,vy:0,onGround:true},
  obstacles:[],particles:[],clouds:[],last:0,beatTimes:[],beatIndex:0,lastBeat:-1,beatPulse:0,
  bpm:120, obstacleBeatCounter:0, analyzedBeats:[], analysisReady:false, audioCtx:null, analyser:null, source:null, data:null, prevData:null, energyHistory:[], fluxHistory:[], detector:null, lastAutoBeat:-10, autoIntervals:[], autoLastPeak:null,
...
async function start(){
  setupAudio(); await state.audioCtx.resume();
  state.score=0;state.combo=0;state.bestCombo=0;state.hitBeats=0;state.obstacles=[];state.particles=[];
  state.beatIndex=0;state.lastBeat=-1;state.obstacleBeatCounter=0;state.dead=false;state.paused=false;state.jumpBuffer=0;state.coyote=0;state.jumpHeld=false;
  state.dino.y=ground()-state.dino.h;state.dino.vy=0;state.dino.onGround=true;state.dino.airTime=0;state.dino.landSquash=0;
  ...
  if(mode==="bpm"){state.bpm=clamp(+$("#bpmInput").value||120,40,240);buildBpmBeats()}
    ...
    $("#startBtn").textContent="ANALISANDO A MÚSICA…"; $("#startBtn").disabled=true;
    try{await analyzeSong();}catch(e){console.warn(e);state.beatTimes=[];state.bpm=120;}
    $("#startBtn").textContent="COMEÇAR JOGO"; $("#startBtn").disabled=false;
  }
...
async function analyzeSong(){
  ...
  // Analisa o arquivo inteiro antes da partida: muito mais estável que tentar adivinhar a batida a cada frame.
  ...
  const hop=Math.max(256,Math.floor(sr*0.04)), win=Math.max(hop*2,Math.floor(sr*0.08));
  const energies=[], flux=[]; let prev=0;
  for(let i=0;i+win<ch.length;i+=hop){
    let sum=0; for(let j=0;j<win;j+=8){const v=ch[i+j]||0;sum+=v*v}
    const e=Math.sqrt(sum/Math.ceil(win/8)); energies.push(e); flux.push(Math.max(0,e-prev)); prev=e;
  }
  const mean=a=>a.reduce((x,y)=>x+y,0)/Math.max(1,a.length);
  const med=a=>{const b=[...a].sort((x,y)=>x-y);return b[Math.floor(b.length/2)]||0};
  const base=med(flux), peaks=[];
  const radius=10;
  for(let i=radius;i<flux.length-radius;i++){
    const local=mean(flux.slice(i-radius,i+radius+1));
    if(flux[i]>Math.max(base*2.0,local*1.45)&&flux[i]>=flux[i-1]&&flux[i]>flux[i+1]) peaks.push(i*hop/sr);
  }
  // Remove picos muito próximos e encontre o BPM dominante a partir dos intervalos.
  const clean=[]; for(const t of peaks){if(!clean.length||t-clean[clean.length-1]>.18)clean.push(t)}
  const hist=new Map();
  for(let i=1;i<clean.length;i++){let d=clean[i]-clean[i-1]; if(d>=.25&&d<=1.0){let bpm=60/d; while(bpm<70)bpm*=2;while(bpm>180)bpm/=2;let k=Math.round(bpm);hist.set(k,(hist.get(k)||0)+1)}}
  let best=120,bv=-1; for(const [b,v] of hist)if(v>bv){best=b;bv=v}
  state.bpm=clamp(best,70,180);
  const step=60/state.bpm;
  // Usa os picos reais quando existirem; preenche buracos com uma grade baseada no BPM.
  const duration=decoded.duration, beats=[];
  let phase=clean.length?clean[0]:0;
  for(let t=phase;t<duration+.1;t+=step)beats.push(t);
  for(const t of clean){if(!beats.some(b=>Math.abs(b-t)<step*.20))beats.push(t)}
  beats.sort((a,b)=>a-b); state.beatTimes=beats; state.beatIndex=0; state.analysisReady=true;
  $("#bpmLabel").textContent=`BPM ${Math.round(state.bpm)}`;
}
...
function jump(){state.jumpBuffer=.14;if(state.dino.onGround||state.coyote>0){state.dino.vy=state.jump;state.dino.onGround=false;state.coyote=0;state.jumpBuffer=0;burst(state.dino.x+20,ground()-5,7)}}
...
function beatNow(){
  ...
  if(mode==="auto"){
    while(state.beatIndex<state.beatTimes.length && state.beatTimes[state.beatIndex]<t-.10) state.beatIndex++;
    if(state.beatIndex<state.beatTimes.length && Math.abs(state.beatTimes[state.beatIndex]-t)<.09){state.beatIndex++;return true}
    return false;
  }
...
function onBeat(){
  ...
  state.obstacleBeatCounter++;
  // No Fácil, nem toda batida vira obstáculo. Isso deixa espaço para aprender o ritmo.
  const diff=$("#difficulty").value;
  const every=diff==="easy"?2:diff==="normal"?1.5:1;
  if(state.obstacleBeatCounter>=every){state.obstacleBeatCounter=0;spawnObstacle();}
}
function spawnObstacle(){
  const last=state.obstacles[state.obstacles.length-1];
  const diff=$("#difficulty").value;
  const minGap=diff==="easy"?Math.max(180,state.speed*24):diff==="normal"?Math.max(145,state.speed*20):Math.max(120,state.speed*17);
  if(last&&last.x>canvas.clientWidth+25-minGap)return;
  const beatNo=state.hitBeats+state.obstacles.length;
  const r=Math.random(); let type="single";
  if(diff!=="easy" && r>.90)type="tall"; else if(diff!=="easy" && r>.76)type="double";
  const h=type==="tall"?52:type==="double"?36:29,w=type==="double"?48:25;
  state.obstacles.push({x:canvas.clientWidth+25,y:ground()-h,w,h,type,passed:false,beat:beatNo});
}
function loop(now){
...
function update(dt){
  const mult={easy:.62,normal:.82,hard:1.05,insane:1.28}[$("#difficulty").value]||1;
  state.speed=(state.baseSpeed+Math.min(5,audio.currentTime*.07))*mult;
  ...
  if(!state.jumpHeld&&state.dino.vy< -260)state.dino.vy+=state.gravity*1.8*dt;
...
function hit(a,b){const ax=a.x+11,ay=a.y+8,aw=a.w-20,ah=a.h-12;return ax<b.x+b.w&&ax+aw>b.x&&ay<b.y+b.h&&ay+ah>b.y}
