...
const state={
  ...
  bpm:120, audioCtx:null, analyser:null, source:null, data:null, detector:null, autoNext:0,
...
async function start(){
  ...
  state.score=0;state.combo=0;state.bestCombo=0;state.hitBeats=0;state.obstacles=[];state.particles=[];state.beatIndex=0;state.lastBeat=-1;state.dead=false;state.paused=false;
...
addEventListener("keydown",e=>{if((e.code==="Space"||e.code==="ArrowUp")&&$("#game").classList.contains("active")){e.preventDefault();jump()}});
canvas.addEventListener("pointerdown",()=>{if($("#game").classList.contains("active")&&!state.paused&&!state.dead)jump()});
function jump(){if(state.dino.onGround){state.dino.vy=state.jump;state.dino.onGround=false;burst(state.dino.x+20,ground()-5,5)}}
...
function detectAuto(){state.analyser.getByteFrequencyData(state.data);let sum=0;for(let i=2;i<Math.min(70,state.data.length);i++)sum+=state.data[i];let energy=sum/68;state.detector=state.detector==null?energy:state.detector*.94+energy*.06;let threshold=Math.max(42,state.detector*1.55);return energy>threshold&&energy>state.autoNext}
function beatNow(){
  const t=audio.currentTime,mode=$("#syncMode").value;
  if(mode==="auto"){
    if(detectAuto()){state.autoNext=state.data.reduce((a,v,i)=>i>0&&v>state.data[a]?i:a,0)+5;return true}
    return false
  }
...
function spawnObstacle(){
  const r=Math.random(),type=r>.82?"tall":r>.62?"double":"single";
  const h=type==="tall"?55:type==="double"?38:32,w=type==="double"?52:28;
...
function update(dt){
  ...
  state.dino.vy+=state.gravity*dt;state.dino.y+=state.dino.vy*dt;
  const g=ground()-state.dino.h;if(state.dino.y>=g){state.dino.y=g;state.dino.vy=0;state.dino.onGround=true}
...
function drawDino(x,y,w,h){
  ctx.save();ctx.translate(x,y);ctx.fillStyle="#f8edf2";
